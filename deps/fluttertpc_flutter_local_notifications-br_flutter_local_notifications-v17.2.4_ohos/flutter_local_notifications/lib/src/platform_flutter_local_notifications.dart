...
import 'package:flutter_local_notifications/src/platform_specifics/ohos/method_channel_mappers.dart';
...
/// Ohos implementation of the local notifications plugin.
class OhosFlutterLocalNotificationsPlugin
    extends MethodChannelFlutterLocalNotificationsPlugin {
  DidReceiveNotificationResponseCallback? _ondidReceiveNotificationResponse;

  Future<bool> initialize(OhosInitializationSettings initializationSettings,
  ...
  Future<void> zonedSchedule(
    ...
    await _channel.invokeMethod(
      ...
      <String, Object?>{
        ...
        'title': title,
        'body': body,
        'platformSpecifics': _buildPlatformSpecifics(notificationDetails),
        'payload': payload ?? '',
        ...scheduledDate.toMap(),
        if (matchDateTimeComponents != null)
          'matchDateTimeComponents': matchDateTimeComponents.index
  ...
  Future<void> periodicallyShow(
      ...
      {OhosNotificationDetails? notificationDetails, String? payload}) async {
    validateId(id);
    await _channel.invokeMethod('periodicallyShow', <String, Object?>{
      'id': id,
      'title': title,
  ...
  Future<void> addNotificationSlot(OhosNotificationSlot notificationSlot) =>
      _channel.invokeMethod('addNotificationSlot', notificationSlot.toMap());

  Future<void> deleteNotificationSlot(OhosNotificationSlotType slotType) =>
      _channel.invokeMethod('deleteNotificationSlot', slotType.value);

  Future<List<OhosNotificationSlot>?> getNotificationSlots() async {
    final List<Map<dynamic, dynamic>>? notificationSlots =
        await _channel.invokeListMethod('getNotificationSlots');

    return notificationSlots?.map((a) {
      List<dynamic> vibrationPattern = a['vibrationPattern'];
      return OhosNotificationSlot(
        OhosNotificationSlotType.values
            .firstWhere((i) => i.value == a['slotType']),
        description: a['description'],
        showBadge: a['showBadge'],
        importance:
            OhosImportance.values.firstWhere((i) => i.value == a['importance']),
        sound: _getNotificationSlotSound(a),
        enableLights: a['enableLights'],
        enableVibration: a['enableVibration'],
        vibrationPattern:
  ...
  OhosNotificationSound? _getNotificationSlotSound(
      Map<dynamic, dynamic> channelMap) {
    final int? soundSourceIndex = channelMap['soundSource'];
    ...
    if (soundSourceIndex != null) {
      if (soundSourceIndex ==
          AndroidNotificationSoundSource.rawResource.index) {
        sound = RawResourceOhosNotificationSound(channelMap['sound']);
      } else if (soundSourceIndex == AndroidNotificationSoundSource.uri.index) {
        sound = UriOhosNotificationSound(channelMap['sound']);
      }
    }
    return sound;
  }
