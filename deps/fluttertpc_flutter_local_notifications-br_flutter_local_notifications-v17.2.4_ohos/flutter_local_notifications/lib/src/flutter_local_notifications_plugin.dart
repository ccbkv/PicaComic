...
class FlutterLocalNotificationsPlugin {
  ...
  T? resolvePlatformSpecificImplementation<
    ...
    if ((defaultTargetPlatform == TargetPlatform.android) &&
      ...
      return FlutterLocalNotificationsPlatform.instance as T?;
        ...
        T == OhosFlutterLocalNotificationsPlugin &&
        FlutterLocalNotificationsPlatform.instance
        is OhosFlutterLocalNotificationsPlugin) {
      return FlutterLocalNotificationsPlatform.instance as T?;
    }
    return null;
  }
  ...
  Future<bool?> initialize(
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      ...
      if (initializationSettings.ohos == null) {
        throw ArgumentError(
            'Ohos settings must be set when targeting Android platform.');
      }

      return resolvePlatformSpecificImplementation<
          OhosFlutterLocalNotificationsPlugin>()
          ?.initialize(
        initializationSettings.ohos!,
        onDidReceiveNotificationResponse: onDidReceiveNotificationResponse
      );
    } else if (defaultTargetPlatform == TargetPlatform.iOS) {
  ...
  Future<void> show(
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      ...
      await resolvePlatformSpecificImplementation<
          OhosFlutterLocalNotificationsPlugin>()
          ?.show(id, title, body,
          notificationDetails: notificationDetails?.ohos,
          payload: payload);
    } else if (defaultTargetPlatform == TargetPlatform.iOS) {
      await resolvePlatformSpecificImplementation<
              IOSFlutterLocalNotificationsPlugin>()
  ...
  Future<void> periodicallyShowWithDuration(
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      ...
      await resolvePlatformSpecificImplementation<
          ...
          ?.periodicallyShowWithDuration(
              id, title, body, repeatDurationInterval,
              notificationDetails: notificationDetails.ohos, payload: payload);
    } else {
      await FlutterLocalNotificationsPlatform.instance
          .periodicallyShowWithDuration(
              id, title, body, repeatDurationInterval);
