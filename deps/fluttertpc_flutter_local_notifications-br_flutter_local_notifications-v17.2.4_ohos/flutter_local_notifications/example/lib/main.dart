...
Future<void> main() async {
  ...
  const OhosInitializationSettings initializationSettingsOhos =
      OhosInitializationSettings('app_icon');
...
Future<void> _configureLocalTimeZone() async {
  ...
  if (Platform.operatingSystem == 'ohos') {
    final String? timeZoneName = await flutterLocalNotificationsPlugin.getLocalTimezone();
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _requestPermissions() async {
    if (Platform.isIOS || Platform.isMacOS) {
      ...
      final OhosFlutterLocalNotificationsPlugin? androidImplementation =
      flutterLocalNotificationsPlugin.resolvePlatformSpecificImplementation<
          OhosFlutterLocalNotificationsPlugin>();
                ...
                children: <Widget>[
                  ...
                  PaddedElevatedButton(
                    buttonText:
                        ...
                        'payload${defaultTargetPlatform == TargetPlatform.ohos ? "(api11 or api12 Unsupported)" : ""}',
                    onPressed: () async {
                  ...
                  if (kIsWeb || !Platform.isLinux) ...<Widget>[
                    PaddedElevatedButton(
                      buttonText:
                          ...
                          'based on local time zone${defaultTargetPlatform == TargetPlatform.ohos ? "(Not a system app, not supported)" : ""}',
                      onPressed: () async {
                    ...
                    PaddedElevatedButton(
                      buttonText:
                          'Schedule notification to appear in 5 seconds '
                          'based on local time zone using alarm clock${defaultTargetPlatform == TargetPlatform.ohos ? "(Not a system app, not supported)" : ""}',
                      onPressed: () async {
                    ...
                    PaddedElevatedButton(
                      buttonText:
                          'Repeat notification every minute${defaultTargetPlatform == TargetPlatform.ohos ? "(Not a system app, not supported)" : ""}',
                      onPressed: () async {
                        await _repeatNotification();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: 'Repeat notification every 5 minutes${defaultTargetPlatform == TargetPlatform.ohos ? "(Not a system app, not supported)" : ""}',
                      onPressed: () async {
                    ...
                    PaddedElevatedButton(
                      buttonText:
                          ...
                          'local time zone${defaultTargetPlatform == TargetPlatform.ohos ? "(Not a system app, not supported)" : ""}',
                      onPressed: () async {
                    ...
                    PaddedElevatedButton(
                      buttonText:
                          'Schedule daily 10:00:00 am notification in your '
                          "local time zone using last year's date${defaultTargetPlatform == TargetPlatform.ohos ? "(Not a system app, not supported)" : ""}",
                      onPressed: () async {
                    ...
                    PaddedElevatedButton(
                      buttonText:
                          ...
                          'in your local time zone${defaultTargetPlatform == TargetPlatform.ohos ? "(Not a system app, not supported)" : ""}',
                      onPressed: () async {
                  ...
                  PaddedElevatedButton(
                    buttonText:
                        ...
                        'your local time zone${defaultTargetPlatform == TargetPlatform.ohos ? "(Not a system app, not supported)" : ""}',
                    onPressed: () async {
                  ...
                  if (!Platform.isLinux)
                    PaddedElevatedButton(
                      buttonText:
                          'Show notification with text choice${defaultTargetPlatform == TargetPlatform.ohos ? "(api11 or api12 Unsupported)" : ""}',
                      onPressed: () async {
                  ...
                  if (Platform.isAndroid) ...<Widget>[
                    ...
                    PaddedElevatedButton(
                      ...
                      onPressed: _areNotifcationsEnabled,
                    ),
                    PaddedElevatedButton(
                  ...
                  if (Platform.operatingSystem == 'ohos') ...<Widget>[
                    const Text(
                      'HarmongOS-specific examples',
                      style: TextStyle(fontWeight: FontWeight.bold),
                    ),
                    Text('notifications enabled: $_notificationsEnabled'),
                    PaddedElevatedButton(
                    ...
                    PaddedElevatedButton(
                      buttonText:
                          'Show notification with custom vibration pattern, '
                          'red LED and red icon${defaultTargetPlatform == TargetPlatform.ohos ? "(color Unsupported)" : ""}',
                      onPressed: () async {
                        await _showNotificationCustomVibrationIconLed();
                      },
                    ),
                    ...
                    PaddedElevatedButton(
                      buttonText:
                          'Show messaging notification${defaultTargetPlatform == TargetPlatform.ohos ? "(Unsupported)" : ""}',
                      onPressed: () async {
                        await _showMessagingNotification();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText:
                          'Show LiveView notification${defaultTargetPlatform == TargetPlatform.ohos ? "(api11 or api12 Unsupported)" : ""}',
                      onPressed: () async {
                        await _showLiveViewNotification();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: 'Show grouped notifications',
                    ...
                    PaddedElevatedButton(
                      buttonText: 'Show ongoing notification${defaultTargetPlatform == TargetPlatform.ohos ? "(ongoing Unsupported)" : ""}',
                      onPressed: () async {
                        await _showOngoingNotification();
                      },
                    ),
                    ...
                    PaddedElevatedButton(
                      buttonText: 'Show notification with chronometer${defaultTargetPlatform == TargetPlatform.ohos ? "(chronometern Unsupported)" : ""}',
                      onPressed: () async {
                        await _showNotificationWithChronometer();
                      },
                    ),
                    ...
                    PaddedElevatedButton(
                      buttonText: 'Show bypassDnd notification',
                      onPressed: () async {
                        await _showNotificationBypassDnd();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: 'Show no float icon notification${defaultTargetPlatform == TargetPlatform.ohos ? "(no float icon Unsupported)" : ""}',
                      onPressed: () async {
                        await _showNotificationIsFloatingIcon();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: 'Create notification channel',
                      onPressed: () async {
                        await _addNotificationSlot();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: 'Delete notification channel',
                      onPressed: () async {
                        await _deleteNotificationSlot();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: 'Get notification channels',
                      onPressed: () async {
                        await _getNotificationSlots();
                      },
                    ),
                  ],
                  if (!kIsWeb &&
  ...
  Future<void> _showNotification() async {
    ...
    const NotificationDetails notificationDetails =
        NotificationDetails(android: androidNotificationDetails, ohos: OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ));
  ...
  Future<void> _showNotificationWithActions() async {
    ...
    const OhosNotificationDetails ohosNotificationDetails =
    OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
      slotDesc: 'your channel description',
      importance: OhosImportance.high,
      actions: <OhosNotificationAction>[
    ...
    OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
      ...
      actions: <OhosNotificationAction>[
        OhosNotificationAction(
          'text_id_1',
          'Enter Text',
          inputs: <OhosNotificationActionInput>[
            OhosNotificationActionInput(),
          ],
        ),
      ],
    );
  ...
  Future<void> _showNotificationWithNoBody() async {
    ...
    const OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            ...
            importance: OhosImportance.high);
    const NotificationDetails notificationDetails = NotificationDetails(
        android: androidNotificationDetails, ohos: ohosNotificationDetails);
    await flutterLocalNotificationsPlugin.show(
  ...
  Future<void> _showNotificationCustomSound() async {
    ...
    const OhosNotificationDetails ohosNotificationDetails =
      ...
      slotDesc: 'your other channel description',
      importance: OhosImportance.high,
      sound: RawResourceOhosNotificationSound('slow_spring_board'),
    );
    final NotificationDetails notificationDetails = NotificationDetails(
      android: androidNotificationDetails,
      iOS: darwinNotificationDetails,
      macOS: darwinNotificationDetails,
      linux: linuxPlatformChannelSpecifics, ohos: ohosNotificationDetails
    );
    await flutterLocalNotificationsPlugin.show(
      id++,
  ...
  Future<void> _showNotificationCustomVibrationIconLed() async {
    ...
    final List<int> vibrationPatternOhos = [0, 1000, 5000, 2000];
    ...
    final AndroidNotificationDetails androidNotificationDetails =
        AndroidNotificationDetails(
            ...
            ledOnMs: 1000,
            ledOffMs: 500);
    final OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            slotDesc: 'other custom channel description',
            icon: 'secondary_icon',
            largeIcon: const DrawableResourceOhosPixelMap('sample_large_icon'),
            vibrationPattern: vibrationPatternOhos,
            enableLights: true,
            colorized: true,
  ...
  Future<void> _zonedScheduleAlarmClockNotification() async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
        ...
        const NotificationDetails(
            ...
            ohos: OhosNotificationDetails(
                OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
                slotDesc: 'Alarm Clock Notification')),
  ...
  Future<void> _showNotificationWithNoSound() async {
    ...
    const OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.CONTENT_INFORMATION,
            slotDesc: 'silent channel description',
            playSound: false,
            styleInformation: OhosDefaultStyleInformation(true, true));
    const DarwinNotificationDetails darwinNotificationDetails =
        DarwinNotificationDetails(
      presentSound: false,
    );
  ...
  Future<void> _showBigPictureNotification() async {
    ...
    if (defaultTargetPlatform == TargetPlatform.ohos) {
      bigPicturePath = await _downloadAndSaveFile(
          'https://dummyimage.com/400x400', 'bigPicture');
    } else {
      bigPicturePath = await _downloadAndSaveFile(
    ...
    final OhosBigPictureStyleInformation bigPictureStyleInformationOhos =
    ...
    final OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            largeIcon: const DrawableResourceOhosPixelMap('sample_large_icon'),// FilePathOhosPixelMap(largeIconPath),
            slotDesc: 'big text channel description',
            styleInformation: bigPictureStyleInformationOhos);
    final NotificationDetails notificationDetails = NotificationDetails(
        android: androidNotificationDetails, ohos: ohosNotificationDetails);
    await flutterLocalNotificationsPlugin.show(
        id++, 'big picture title', 'big picture body', notificationDetails);
  }
  ...
  Future<void> _showBigPictureNotificationBase64() async {
    ...
    String bigPicture = '';
    if (defaultTargetPlatform == TargetPlatform.ohos) {
      bigPicture = await _base64encodedImage('https://dummyimage.com/400x400');
    } else {
      bigPicture = await _base64encodedImage('https://dummyimage.com/400x800');
    }
    final BigPictureStyleInformation bigPictureStyleInformation =
    ...
    final AndroidNotificationDetails androidNotificationDetails =
        AndroidNotificationDetails(
            ...
            styleInformation: bigPictureStyleInformation);
    final OhosBigPictureStyleInformation bigPictureStyleInformationOhos =
        OhosBigPictureStyleInformation(
            ByteArrayOhosPixelMap.fromBase64String(
                bigPicture),
            contentTitle: 'overridden <b>big</b> content title',
            htmlFormatContentTitle: true,
            summaryText: 'summary <i>text</i>',
    ...
    final OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            largeIcon: ByteArrayOhosPixelMap.fromBase64String(largeIcon),
            slotDesc: 'big text channel description',
            styleInformation: bigPictureStyleInformationOhos);
    final NotificationDetails notificationDetails = NotificationDetails(
        android: androidNotificationDetails, ohos: ohosNotificationDetails);
  ...
  Future<void> _showBigPictureNotificationURL() async {
    ...
    Uint8List bigPictureArray = Uint8List(0);
    if (defaultTargetPlatform == TargetPlatform.ohos) {
      bigPictureArray =
          await _getByteArrayFromUrl('https://dummyimage.com/400x400');
    } else {
      bigPictureArray =
          await _getByteArrayFromUrl('https://dummyimage.com/400x800');
    }
    final ByteArrayAndroidBitmap largeIcon =
        ByteArrayAndroidBitmap(largeIconArray);
    final ByteArrayAndroidBitmap bigPicture =
        ByteArrayAndroidBitmap(bigPictureArray);
    ...
    final AndroidNotificationDetails androidNotificationDetails =
        AndroidNotificationDetails(
            ...
            channelDescription: 'big text channel description',
            styleInformation: bigPictureStyleInformation);
    final ByteArrayOhosPixelMap largeIconOhos =
        ByteArrayOhosPixelMap(largeIconArray);
    final ByteArrayOhosPixelMap bigPictureOhos =
        ByteArrayOhosPixelMap(bigPictureArray);
    final OhosBigPictureStyleInformation bigPictureStyleInformationOhos =
        OhosBigPictureStyleInformation(bigPictureOhos,
            contentTitle: 'overridden <b>big</b> content title',
            htmlFormatContentTitle: true,
            summaryText: 'summary <i>text</i>',
            htmlFormatSummaryText: true);
  ...
  Future<void> _showBigTextNotification() async {
    ...
    const OhosBigTextStyleInformation bigTextStyleInformationOhos =
        OhosBigTextStyleInformation(
      'Lorem <i>ipsum dolor sit</i> amet, consectetur <b>adipiscing elit</b>, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.',
      htmlFormatBigText: true,
      contentTitle: 'overridden <b>big</b> content title',
      htmlFormatContentTitle: true,
    ...
    const OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            slotDesc: 'big text channel description',
            styleInformation: bigTextStyleInformationOhos);
    const NotificationDetails notificationDetails = NotificationDetails(
        android: androidNotificationDetails, ohos: ohosNotificationDetails);
    await flutterLocalNotificationsPlugin.show(
        id++, 'big text title', 'silent body', notificationDetails);
  ...
  Future<void> _showInboxNotification() async {
    ...
    final OhosInboxStyleInformation inboxStyleInformationOhos =
        OhosInboxStyleInformation(lines,
            htmlFormatLines: true,
            contentTitle: 'overridden <b>inbox</b> context title',
            htmlFormatContentTitle: true,
            summaryText: 'summary <i>text</i>',
    ...
    final OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            slotDesc: 'inbox channel description',
            styleInformation: inboxStyleInformationOhos);
    final NotificationDetails notificationDetails = NotificationDetails(
        android: androidNotificationDetails, ohos: ohosNotificationDetails);
    await flutterLocalNotificationsPlugin.show(
  ...
  Future<void> _showLiveViewNotification() async {
    // TODO 当前api12版本，三方app暂不生效未实现
  }
  ...
  Future<void> _showGroupedNotifications() async {
    ...
    const AndroidNotificationDetails firstNotificationAndroidSpecifics =
        AndroidNotificationDetails(groupChannelId, groupChannelName,
            ...
            priority: Priority.high,
            groupKey: groupKey);
    const OhosNotificationDetails firstNotificationOhosSpecifics =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            slotDesc: groupChannelDescription,
            importance: OhosImportance.high,
            groupKey: groupKey);
    const NotificationDetails firstNotificationPlatformSpecifics =
        NotificationDetails(
            android: firstNotificationAndroidSpecifics,
            ohos: firstNotificationOhosSpecifics);
    ...
    const AndroidNotificationDetails secondNotificationAndroidSpecifics =
        AndroidNotificationDetails(groupChannelId, groupChannelName,
            ...
            priority: Priority.high,
            groupKey: groupKey);
    const OhosNotificationDetails secondNotificationOhosSpecifics =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            slotDesc: groupChannelDescription,
            importance: OhosImportance.high,
            groupKey: groupKey);
    const NotificationDetails secondNotificationPlatformSpecifics =
        NotificationDetails(
            ...
            ohos: secondNotificationOhosSpecifics);
    await flutterLocalNotificationsPlugin.show(
        id++,
        'Jeff Chang',
    ...
    const OhosInboxStyleInformation inboxStyleInformationOhos =
        OhosInboxStyleInformation(lines,
  ...
  Future<void> _showNotificationWithTag() async {
    ...
    const NotificationDetails notificationDetails = NotificationDetails(
        android: androidNotificationDetails, ohos: ohosNotificationDetails);
    await flutterLocalNotificationsPlugin.show(id++, 'first notification',
        'first notification body', notificationDetails);
  }
  ...
  Future<void> _repeatNotification() async {
    ...
    const OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            slotDesc: 'repeating description');
    const NotificationDetails notificationDetails = NotificationDetails(
        android: androidNotificationDetails, ohos: ohosNotificationDetails);
  ...
  Future<void> _repeatPeriodicallyWithDurationNotification() async {
    ...
    const OhosNotificationDetails ohosNotificationDetails = OhosNotificationDetails(
        ...
        slotDesc: 'repeating description');
    const NotificationDetails notificationDetails =
        NotificationDetails(android: androidNotificationDetails, ohos: ohosNotificationDetails);
  ...
  Future<void> _scheduleDailyTenAMNotification() async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
        ...
        const NotificationDetails(
            ...
            ohos: OhosNotificationDetails(
                OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
                slotDesc: 'daily notification description')),
        androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
        uiLocalNotificationDateInterpretation:
            UILocalNotificationDateInterpretation.absoluteTime,
  ...
  Future<void> _scheduleWeeklyTenAMNotification() async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
        ...
        const NotificationDetails(
            ...
            ohos: OhosNotificationDetails(
                OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
                slotDesc: 'weekly notificationdescription')),
        androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
        uiLocalNotificationDateInterpretation:
            UILocalNotificationDateInterpretation.absoluteTime,
  ...
  Future<void> _scheduleMonthlyMondayTenAMNotification() async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
        ...
        const NotificationDetails(
            ...
            ohos: OhosNotificationDetails(
                OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
                slotDesc: 'monthly notificationdescription')),
        androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
        uiLocalNotificationDateInterpretation:
            UILocalNotificationDateInterpretation.absoluteTime,
  ...
  Future<void> _scheduleYearlyMondayTenAMNotification() async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
        ...
        const NotificationDetails(
            ...
            ohos: OhosNotificationDetails(
                OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
                slotDesc: 'yearly notification description')),
        androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
        uiLocalNotificationDateInterpretation:
            UILocalNotificationDateInterpretation.absoluteTime,
    ...
    for (int i = 0; i <= maxProgress; i++) {
      await Future<void>.delayed(const Duration(seconds: 1), () async {
        ...
        final OhosNotificationDetails ohosNotificationDetails =
            OhosNotificationDetails(
                OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
                slotDesc: 'progress channel description',
                slotShowBadge: false,
                importance: OhosImportance.high,
                onlyAlertOnce: true,
                showProgress: true,
  ...
  Future<void> _showIndeterminateProgressNotification() async {
    ...
    const OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            slotDesc: 'indeterminate progress channel description',
            slotShowBadge: false,
            importance: OhosImportance.high,
            onlyAlertOnce: true,
            showProgress: true,
  ...
  Future<void> _showNotificationUpdateChannelDescription() async {
    ...
    const OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            slotDesc: 'your updated channel description',
            importance: OhosImportance.high,
            slotAction: OhosNotificationSlotAction.update);
    const NotificationDetails notificationDetails = NotificationDetails(
        android: androidNotificationDetails, ohos: ohosNotificationDetails);
    await flutterLocalNotificationsPlugin.show(
        id++,
  ...
  Future<void> _showNotificationWithChronometer() async {
    ...
    final OhosLiveViewStyleInformation liveViewStyleInformation =
        OhosLiveViewStyleInformation('title', 'text', 1,
            initialTime: 120000, isCountDown: true, isInTitle: true);
    final OhosNotificationDetails ohosNotificationDetails =
        OhosNotificationDetails(
      OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
      slotDesc: 'your channel description',
      ...
      usesChronometer: true,
      chronometerCountDown: true,
      // styleInformation: liveViewStyleInformation
    );
    final NotificationDetails notificationDetails = NotificationDetails(
        android: androidNotificationDetails, ohos: ohosNotificationDetails);
    await flutterLocalNotificationsPlugin.show(
  ...
  Future<void> _addNotificationSlot() async {
    const OhosNotificationSlot ohosNotificationSlot = OhosNotificationSlot(
      OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
      description: 'your slot description',
    );
    await flutterLocalNotificationsPlugin
        .resolvePlatformSpecificImplementation<
            OhosFlutterLocalNotificationsPlugin>()
        ?.addNotificationSlot(ohosNotificationSlot);

    await showDialog<void>(
        context: context,
        builder: (BuildContext context) => AlertDialog(
              content:
                  Text('Slot with name ${ohosNotificationSlot.slotType.name} '
                      'created'),
              actions: <Widget>[
                TextButton(
                  onPressed: () {
  ...
  Future<void> _deleteNotificationSlot() async {
    const OhosNotificationSlotType slotType =
        OhosNotificationSlotType.SOCIAL_COMMUNICATION ;
    await flutterLocalNotificationsPlugin
        .resolvePlatformSpecificImplementation<
            OhosFlutterLocalNotificationsPlugin>()
        ?.deleteNotificationSlot(slotType);
    await showDialog<void>(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        content: Text('Channel with id ${slotType.name} deleted'),
        actions: <Widget>[
          TextButton(
            onPressed: () {
              Navigator.of(context).pop();
  ...
  Future<void> _getNotificationSlots() async {
    final Widget notificationChannelsDialogContent =
        await _getNotificationSlotsDialogContent();
    await showDialog<void>(
      context: context,
      builder: (BuildContext context) => AlertDialog(
  ...
  Future<Widget> _getNotificationSlotsDialogContent() async {
    try {
      final List<OhosNotificationSlot>? slots =
          await flutterLocalNotificationsPlugin
              .resolvePlatformSpecificImplementation<
                  OhosFlutterLocalNotificationsPlugin>()!
        ...
        child: ListView(
          children: <Widget>[
            ...
            if (slots?.isEmpty ?? true)
              const Text('No notification Slots')
            else
              for (final OhosNotificationSlot slot in slots!)
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    Text('slotType: ${slot.slotType.name}\n'
                        'description: ${slot.description}\n'
                        'importance: ${slot.importance.value}\n'
                        'playSound: ${slot.playSound}\n'
                        'sound: ${slot.sound?.sound}\n'
                        'enableVibration: ${slot.enableVibration}\n'
                        'vibrationPattern: ${slot.vibrationPattern}\n'
                        'showBadge: ${slot.showBadge}\n'
                        'enableLights: ${slot.enableLights}\n'
      ...
      return Text(
        'Error calling "getNotificationSlots"\n'
        'code: ${error.code}\n'
  ...
  Future<void> _areNotifcationsEnabled() async {
    bool? areEnabled = false;
    if (Platform.isAndroid) {
      areEnabled = await flutterLocalNotificationsPlugin
          .resolvePlatformSpecificImplementation<
              AndroidFlutterLocalNotificationsPlugin>()
          ?.areNotificationsEnabled();
    } else if (Platform.operatingSystem == 'ohos') {
    ...
    setState(() {
      _notificationsEnabled = areEnabled ?? false;
    });
    await showDialog<void>(
        context: context,
        builder: (BuildContext context) => AlertDialog(
  ...
  Future<void> _showNotificationWithNumber() async {
    const AndroidNotificationDetails androidPlatformChannelSpecifics =
        AndroidNotificationDetails('your channel id', 'your channel name',
            ...
            priority: Priority.high,
            number: 1);
    const OhosNotificationDetails ohosPlatformChannelSpecifics =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            slotDesc: 'your channel description',
            importance: OhosImportance.high,
            badgeNumber: 1);
    const NotificationDetails platformChannelSpecifics = NotificationDetails(
        ...
        ohos: ohosPlatformChannelSpecifics);
    await flutterLocalNotificationsPlugin.show(
        0, 'icon badge title', 'icon badge body', platformChannelSpecifics,
        payload: 'item x');
  }

  Future<void> _showNotificationBypassDnd() async {
    const OhosNotificationDetails ohosPlatformChannelSpecifics =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            bypassDnd: true);
    ...
    await flutterLocalNotificationsPlugin.show(
        0, 'bypassDnd title', 'bypassDnd body', platformChannelSpecifics,
        payload: 'item x');
  }

  Future<void> _showNotificationIsFloatingIcon() async {
    const OhosNotificationDetails ohosPlatformChannelSpecifics =
        OhosNotificationDetails(OhosNotificationSlotType.SOCIAL_COMMUNICATION ,
            isFloatingIcon: false);
    const NotificationDetails platformChannelSpecifics =
        NotificationDetails(ohos: ohosPlatformChannelSpecifics);
    await flutterLocalNotificationsPlugin.show(0, 'no float icon title',
        'no float icon body', platformChannelSpecifics,
        payload: 'item x');
  }
